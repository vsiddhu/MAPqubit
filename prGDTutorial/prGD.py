...
def doPrGDVec(fun, x0, grdFun, maxIter = 15000, grdNrmMax = 1e-8, minIter = 5, stepSize = 0.1, stepMax = 10.0, **kargs):
    """
...
#Add initial results
    flag = prGDFun.isValidBloch(x0)
    if not flag:
        raise ValueError('Starting point computation error: x0 does not represent, a valid density matrix')
    else:
        newPt = x0
